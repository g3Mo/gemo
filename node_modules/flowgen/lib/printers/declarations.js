...
const interfaceType = (node, withSemicolons = false) => {
...
const interfaceRecordDeclaration = (nodeName, node, modifier) => {
  ...
  if (node.heritageClauses) {
    heritage = node.heritageClauses.map(clause => {
      return clause.types.map(type => _index.default.node.printType(type)).map(type => `...$Exact<${type}>`).join(",\n");
    }).join("");
...
const interfaceDeclaration = (nodeName, node, modifier) => {
    ...
    heritage = node.heritageClauses.map(clause => {
      return clause.types.map(type => {
        ...
        if (type.expression.kind === ts.SyntaxKind.Identifier) {
          return _index.default.node.getFullyQualifiedPropertyAccessExpression(symbol, type.expression) + _index.default.common.generics(type.typeArguments);
        } else {
  ...
  let str = `${modifier}${type} ${nodeName}${_index.default.common.generics(node.typeParameters)} ${type === "type" ? "= " : ""}${interfaceType(node)} ${heritage}`;
  return str;
};
